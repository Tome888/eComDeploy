...
export async function getStaticProps() {
  const response = await fetch(
    "https://hallowed-jagged-collision.glitch.me/banner_content"
  );
  const dataBanner = await response.json();

  const responseFeat = await fetch(
    "https://hallowed-jagged-collision.glitch.me/products/?_limit=4"
  );
  const featProductsData = await responseFeat.json();

  const responseBlog = await fetch(
    "https://hallowed-jagged-collision.glitch.me/blogs/?_limit=3"
  );
  const blogsData = await responseBlog.json();
  return {
    props: {
